  ...
  <section class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
      ...
      <div class="flex flex-wrap items-center gap-2">
        <button
          class="border border-slate-700 bg-slate-800 px-3 py-1.5 text-sm hover:bg-slate-700"
          type="button"
          @click="goPrevWeek"
        >
          Previous week
    ...
    <div class="rounded-2xl border border-slate-800 bg-slate-900/80 px-5 py-4">
      ...
      <p class="mt-1 text-sm text-slate-400">
        {{ queueForWeek.length }} submitted timesheet{{ queueForWeek.length === 1 ? '' : 's' }} in
        this week.
      </p>
    </div>

    <div
      v-if="queueForWeek.length === 0"
    ...
    >
      <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p class="mt-1 text-sm text-slate-500">
            Total: {{ totalHours(timesheet).toFixed(2) }} hours
          </p>
        </div>
        ...
        <table class="w-full min-w-[640px] text-left text-sm">
          <thead class="bg-slate-950 text-slate-400">
            <tr>
              ...
              <th
                v-for="day in weekDayColumns(timesheet.periodStart)"
                :key="day.date"
          ...
          <tbody>
            <tr
              v-for="row in buildRows(timesheet)"
              :key="row.taskId"
            ...
            >
              ...
              <td
                v-for="day in weekDayColumns(timesheet.periodStart)"
                :key="day.date"
                class="p-2 text-center tabular-nums text-slate-200"
              >
      ...
      <div class="mt-5 flex justify-end gap-3">
        <button
          ...
          type="button"
          @click="timesheets.reject(timesheet.id)"
        >
          Reject
        </button>
        <button
          class="bg-emerald-500 text-slate-950 hover:bg-emerald-400"
          data-cy="approve-timesheet"
          type="button"
          @click="timesheets.approve(timesheet.id)"
        >
          Approve
        </button>
      </div>
